...
#if CONFIG_LED_ILLUMINATOR_ENABLED
    led_duty = flash_state ? CONFIG_LED_MAX_INTENSITY : 0;
    enable_led(flash_state);
    log_i("Flash toggled: %s, duty: %d", flash_state ? "ON" : "OFF", led_duty);
#endif
...
void setupLedFlash(int pin) 
{
    ...
    log_i("LED flash setup on pin %d, channel %d", pin, LED_LEDC_CHANNEL);
#else
